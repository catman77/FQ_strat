...
class PCA3_short(IStrategy):
    ...
    # Required
    startup_candle_count: int = 132  # must be power of 2
    ...
    curr_lookahead = int(36 * lookahead_hours)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # kst_indicator = pta.kst(dataframe['close'], roc1=self.kst1_length.value, roc2=self.kst2_length.value, roc3=self.kst3_length.value, roc4=self.kst4_length.value, signal=self.kst5_length.value,
        #                 sma1=self.kst6_length.value, sma2=self.kst7_length.value, sma3=self.kst8_length.value, sma4=self.kst9_length.value)

        # Extract the column names from the returned DataFrame
        # kst_column = kst_indicator.columns[0]
        # kst_signal_column = kst_indicator.columns[1]

        # # Assign the calculated KST and KST signal values to the dataframe
        # dataframe['kst'] = kst_indicator[kst_column]
        # dataframe['kst_signal'] = kst_indicator[kst_signal_column]
        # # Thêm cột để kiểm tra hướng của KST
        # dataframe['kst_diff'] = dataframe['kst'].diff()
        # dataframe['upperband'], dataframe['middleband'], dataframe['lowerband'] = talib.BBANDS(dataframe['close'], timeperiod=20, nbdevup=2, nbdevdn=2, matype=0)
    ...
    def add_indicators(self, dataframe: DataFrame) -> DataFrame:
        ...
        # Thêm EMA 200 và EMA 100
        dataframe['ema200'] = ta.EMA(dataframe['close'], timeperiod=200)
        dataframe['ema100'] = ta.EMA(dataframe['close'], timeperiod=100)
        
        # Kiểm tra hướng lên của EMA 200 và EMA 100
        dataframe['ema200_trending_up'] = (dataframe['ema200'].shift(1) < dataframe['ema200']) & (dataframe['ema200'].shift(2) < dataframe['ema200'].shift(1))
        dataframe['ema100_trending_up'] = (dataframe['ema100'].shift(1) < dataframe['ema100']) & (dataframe['ema100'].shift(2) < dataframe['ema100'].shift(1))

        # Kiểm tra hướng xuống của EMA 200 và EMA 100
        dataframe['ema200_trending_down'] = (dataframe['ema200'].shift(1) > dataframe['ema200']) & (dataframe['ema200'].shift(2) > dataframe['ema200'].shift(1))
        dataframe['ema100_trending_down'] = (dataframe['ema100'].shift(1) > dataframe['ema100']) & (dataframe['ema100'].shift(2) > dataframe['ema100'].shift(1))
        # RSI
        period = 14
        smoothD = 3
        SmoothK = 3
        ...
        dataframe['DIF'], dataframe['DEA'], _ = ta.MACD(dataframe['close'], fastperiod=12, slowperiod=26, signalperiod=9)
    ...
    #         # Reduce proposed stake to leave funds for possible further DCA orders
    #         proposed_stake = proposed_stake / self.max_dca_multiplier  # Leaving some reserve in case the market dumps

    #         # print('proposed_stake: ' + str(proposed_stake))

    #         # Adjust proposed stake by dividing it by the leverage factor to maintain risk exposure
    #         base_stake_amount = proposed_stake / self.leverageNumber

    #         # Return the stake amount considering the leverage
    ...
    def add_stoploss_indicators(self, dataframe, pair) -> DataFrame:
        ...
        # Trends
        dataframe['candle_up'] = np.where(dataframe['close'] >= dataframe['close'].shift(1), 1.0, -1.0)
        dataframe['candle_up_seq'] = dataframe['candle_up'].rolling(window=5).sum()
        dataframe['candle_up_trend'] = np.where(dataframe['candle_up_seq'] > 0.0, 1.0, -1.0)

        dataframe['candle_dn'] = np.where(dataframe['close'] < dataframe['close'].shift(1), 1.0, -1.0)
        dataframe['candle_dn_seq'] = dataframe['candle_dn'].rolling(window=5).sum()
        dataframe['candle_dn_trend'] = np.where(dataframe['candle_dn_seq'] > 0.0, 1.0, -1.0)

        dataframe['rmi_up'] = np.where(dataframe['rmi'] >= dataframe['rmi'].shift(1), 1.0, -1.0)
        dataframe['rmi_up_trend'] = np.where(dataframe['rmi_up'].rolling(window=5).sum() > 0.0, 1.0, -1.0)

        dataframe['rmi_dn'] = np.where(dataframe['rmi'] < dataframe['rmi'].shift(1), 1.0, -1.0)
        dataframe['rmi_dn_trend'] = np.where(dataframe['rmi_dn'].rolling(window=5).sum() > 0.0, 1.0, -1.0)          
        ...
        dataframe['ssl_dir_short'] = np.where(sslup < ssldown, 1.0, -1.0)
    ...
    #     fac = self.leverageNumber 
    #     # Profit Based DCA
    #     if current_profit >  -0.0125 * fac and trade.nr_of_successful_entries == 1:
    #         return None
    #     if current_profit > -0.03 * fac and trade.nr_of_successful_entries == 2:
    #         return None
    #     # if current_profit > -0.05 * fac and trade.nr_of_successful_entries == 3:
    #     #     return None
    #     # if current_profit > -0.08 * fac and trade.nr_of_successful_entries == 4:
    #     #     return None
    #     # if current_profit > -0. * fac and trade.nr_of_successful_entries == 5:
    #     #     return None

    #     # Time Based DCA
    #     if current_profit > -0.0125 * fac:
    #         #12 hrs
    ...
    #         # stake_amount = filled_entries[0].cost / self.leverageNumber
    #         # This then calculates current safety order size
    #         if count_of_entries == 1:
    #             print('1 current_profit' + str(current_profit))
    #             stake_amount = stake_amount * 1.125
    #         elif count_of_entries == 2:
    #             print('2 current_profit' + str(current_profit))
    #             stake_amount = stake_amount * 1.25
    #         # elif count_of_entries == 3:
    #         #     print('3 current_profit' + str(current_profit))
    #         #     stake_amount = stake_amount * 1.375
    #         # elif count_of_entries == 4:
    #         #     print('4 current_profit' + str(current_profit))
    #         #     stake_amount = stake_amount * 1.5
    #         # elif count_of_entries == 5:
    #         #     # print('5 current_profit' + str(current_profit))
    #         #     stake_amount = stake_amount * 1.625

    #         # print('stake_amount_DCA: ' + str(stake_amount))
    #         # Apply leverage to the calculated stake amount

    #         return stake_amount / self.leverageNumber
    ...
    # default classifier
    # sigmoidSVC, linearSVC, GradientBoosting
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        enter_long_conditions = (
            ...
            (dataframe['close'] > dataframe['ema100']* 1.002) &  # Use EMA 200 instead of TEMA
            (dataframe['ema100'] > dataframe['ema200']* 1.002) &  # Use EMA 100 to confirm the trend
            (dataframe['ema200_trending_up']) &
            (dataframe['ema100_trending_up']) &
        ...
        enter_short_conditions = (
            ...
            # (dataframe['rsi'] > 70) &
            (dataframe['close'] * 1.002< dataframe['ema100']) &  # Use EMA 200 instead of TEMA
            (dataframe['ema100']* 1.002 < dataframe['ema200']) &  # Use EMA 100 to confirm the trend
            (dataframe['ema200_trending_down']) &
            (dataframe['ema100_trending_down']) &
            # (dataframe['macd'] < dataframe['macdsignal']) &
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        # current_profit = current_profit * self.leverageNumber

        is_short = trade.is_short 
        # print('is_short: ' +str(is_short) )
        dataframe, _ = self.dp.get_analyzed_dataframe(pair=pair, timeframe=self.timeframe)
        last_candle = dataframe.iloc[-1].squeeze()

        if is_short == False:
            trade_dur = int((current_time.timestamp() - trade.open_date_utc.timestamp()) // 60)
            max_profit = max(0, trade.calc_profit_ratio(trade.max_rate)) * self.leverageNumber
            # print('max_profit: ' +str(max_profit) )
            pullback_value = max(0, (max_profit - self.cexit_pullback_amount.value))
            # print('pullback_value: ' +str(pullback_value) )
            in_trend = False
            ...
            # Determine if there is a trend
            if self.cexit_trend_type.value == 'rmi' or self.cexit_trend_type.value == 'any':
                if last_candle['rmi_dn_trend'] == 1:
                    in_trend = True
            if self.cexit_trend_type.value == 'ssl' or self.cexit_trend_type.value == 'any':
                if last_candle['ssl_dir_short'] == 1:
                    in_trend = True
            if self.cexit_trend_type.value == 'candle' or self.cexit_trend_type.value == 'any':
                if last_candle['candle_dn_trend'] == 1:
                    in_trend = True
