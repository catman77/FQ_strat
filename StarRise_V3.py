...
from freqtrade.strategy import merge_informative_pair, CategoricalParameter, DecimalParameter, IntParameter, BooleanParameter, stoploss_from_open
import math
import logging
import datetime
...
class StarRise_strat(IStrategy):
    ...
    # Select Entry Logic
    use0 = BooleanParameter(default=True, space='buy', load=True)
    use1 = BooleanParameter(default=True, space='buy', load=True)
    use2 = BooleanParameter(default=True, space='buy', load=True)
    ...
    wr_buy = IntParameter(-80, -30, default=-30, space='buy', load=True)
    cti_buy = DecimalParameter(0.0, 5.0, default=3.0, decimals=2, space='buy', load=True)
    adx_buy = IntParameter(15, 30, default=26, space='buy', load=True)
    mama1_buy = DecimalParameter(-0.01, 0.01, default= 0.003, decimals=3, space='buy', load=True)
    rsi84_buy = IntParameter(40, 60, default=55, space='buy', load=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['tpct_change_1'] = top_percent_change(dataframe, 10)
        dataframe['tpct_change_2'] = top_percent_change(dataframe, 20)
        dataframe['tpct_change_4'] = top_percent_change(dataframe, 40)
        dataframe['tpct_change_5'] = top_percent_change(dataframe, 50)
        dataframe['tpct_change_9'] = top_percent_change(dataframe, 90)
        dataframe['tpct_change_0_mean'] = dataframe['tpct_change_0'].mean()
        dataframe['tpct_change_1_mean'] = dataframe['tpct_change_1'].mean()
        dataframe['tpct_change_2_mean'] = dataframe['tpct_change_2'].mean()
        dataframe['tpct_change_4_mean'] = dataframe['tpct_change_4'].mean()
        dataframe['tpct_change_5_mean'] = dataframe['tpct_change_5'].mean()
        dataframe['tpct_change_9_mean'] = dataframe['tpct_change_9'].mean()
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        is_dip_0 = (
            ...
            (dataframe['mama_diff_1h'] < self.mama2_buy.value) & 
            (dataframe['mama_diff_1h'] > self.mama1_buy.value) & 
            (dataframe['close'].rolling(288).max() >= dataframe['threshold_mean']) &
            (dataframe['volume'] > 0) &
            (self.use0.value == True) 
        )
        dataframe.loc[is_dip_0, 'enter_long'] = 1
        dataframe.loc[is_dip_0, 'enter_tag'] = 'is dip 0'
        ...
        is_dip_1 = (
            ...
            (dataframe['mama_diff_1h'] < self.mama2_buy.value) & 
            (dataframe['mama_diff_1h'] > self.mama1_buy.value) & 
            (dataframe['close'].rolling(288).max() >= dataframe['threshold_1']) &
            (dataframe['volume'] > 0) &
            (self.use1.value == True) 
        )
        dataframe.loc[is_dip_1, 'enter_long'] = 1
        dataframe.loc[is_dip_1, 'enter_tag'] = 'is dip 1'
        ...
        is_dip_2 = (
            ...
            (dataframe['mama_diff_1h'] < self.mama2_buy.value) & 
            (dataframe['mama_diff_1h'] > self.mama1_buy.value) & 
            (dataframe['close'].rolling(288).max() >= dataframe['threshold_2']) &
            (dataframe['volume'] > 0) &
            (self.use2.value == True) 
        )
        dataframe.loc[is_dip_2, 'enter_long'] = 1
        dataframe.loc[is_dip_2, 'enter_tag'] = 'is dip 2'
        ...
        is_dip_3 = (
            ...
            (dataframe['cti_40_1h'] < self.cti1_buy.value) & 
            (dataframe['r_96_1h'] < self.wr1_buy.value) & 
            (dataframe['mama_diff_1h'] < dataframe['threshold_mean']) & 
            (dataframe['mama_diff_1h'] > self.mama1_buy.value) & 
            (dataframe['close'].rolling(288).max() >= dataframe['threshold_mean']) &
            (dataframe['volume'] > 0) &
            (self.use3.value == True) 
        )
        dataframe.loc[is_dip_3, 'enter_long'] = 1
        dataframe.loc[is_dip_3, 'enter_tag'] = 'is dip 3'
        ...
        is_dip_4 = (
            ...
            (dataframe['cti_40_1h'] < self.cti1_buy.value) & 
            (dataframe['r_96_1h'] < self.wr1_buy.value) & 
            (dataframe['mama_diff_1h'] < dataframe['threshold_1']) & 
            (dataframe['mama_diff_1h'] > self.mama1_buy.value) & 
            (dataframe['close'].rolling(288).max() >= dataframe['threshold_1']) &
            (dataframe['volume'] > 0) &
            (self.use4.value == True) 
        )
        dataframe.loc[is_dip_4, 'enter_long'] = 1
        dataframe.loc[is_dip_4, 'enter_tag'] = 'is dip 4'
